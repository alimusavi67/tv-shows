...
const EpisodeItem: React.FC<EpisodeItemProps> = ({ episode }) => {
  ...
  const sanitizeSummary = episode?.summary ? DOMPurify.sanitize(episode?.summary) : '<em>No summary available</em>';
  return (
    <li key={episode.id} className="p-2 border rounded-lg">
      ...
      <div className="mt-2">
        ...
        <p className="text-sm mt-2" dangerouslySetInnerHTML={{ __html: sanitizeSummary }}></p>
      </div>
    </li>
  );
};
